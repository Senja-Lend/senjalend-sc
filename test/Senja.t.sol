...
contract SenjaTest is Test, Helper {
    ...
    // Using WETH instead of native token address(1) for better DeFi composability
    ...
    function _deployOFT() internal {
        if (block.chainid == 8453) {
            ...
            oftwethadapter = new OFTWETHadapter(WETH, address(elevatedminterburner), BASE_LZ_ENDPOINT, owner);
            base_oftweth_adapter = address(oftwethadapter);
            oapp2 = address(oftwethadapter);
    ...
    function test_repay_debt() public {
        ...
        IERC20(USDT).approve(lendingPool, 10e6);
        ILendingPool(lendingPool).repayWithSelectedToken(10e6, USDT, false, alice, 500);
        IERC20(WETH).approve(lendingPool2, 0.01 ether);
        ILendingPool(lendingPool2).repayWithSelectedToken(0.01 ether, WETH, false, alice, 500);
        IERC20(WETH).approve(lendingPool2, 0.01 ether);
        ILendingPool(lendingPool2).repayWithSelectedToken(0.01 ether, WETH, false, alice, 500);
    ...
    function test_comprehensive_collateral_swap_repay() public {
        ...
        // Step 4: Test swapping collateral (WETH) to borrow token (USDT) with high slippage
        vm.startPrank(alice);

        // Check initial balances
        console.log("Initial WETH in position:", IERC20(WETH).balanceOf(position));
        console.log("Initial USDT in position:", IERC20(USDT).balanceOf(position));

        // Swap WETH to USDT with 10000 slippage tolerance (100%)
        IPosition(position).swapTokenByPosition(WETH, USDT, 100 ether, 10000);

        // Check balances after swap
        console.log("Final WETH in position:", IERC20(WETH).balanceOf(position));
        console.log("Final USDT in position:", IERC20(USDT).balanceOf(position));

        vm.stopPrank();

        // Step 5: Test swapping collateral to WETH (simulated with WETH)
        vm.startPrank(alice);

        // Check balances before second swap
        console.log("Before second swap - WETH:", IERC20(WETH).balanceOf(position));
        console.log("Before second swap - USDT:", IERC20(USDT).balanceOf(position));

        // Swap USDT back to WETH (simulating WETH) with high slippage
        IPosition(position).swapTokenByPosition(USDT, WETH, 10e6, 10000);

        // Check balances after second swap
        console.log("After second swap - WETH:", IERC20(WETH).balanceOf(position));
        ...
        console.log("Before repayment - WETH:", IERC20(WETH).balanceOf(position));
        ...
        console.log("After repayment - WETH:", IERC20(WETH).balanceOf(position));
    ...
    function test_swap_with_extreme_slippage() public {
        ...
        console.log("Initial WETH:", IERC20(WETH).balanceOf(position));
        console.log("Initial USDT:", IERC20(USDT).balanceOf(position));

        // This should work even with extreme slippage
        IPosition(position).swapTokenByPosition(WETH, USDT, swapAmount, 10000);

        console.log("After swap WETH:", IERC20(WETH).balanceOf(position));
        ...
        IPosition(position).swapTokenByPosition(USDT, WETH, usdtAmount, 10000);

        console.log("After swap back WETH:", IERC20(WETH).balanceOf(position));
    ...
    function test_position_repay_with_swap() public {
        ...
        // First, swap some WETH to USDT in the position
        vm.startPrank(alice);

        console.log("Before swap - WETH:", IERC20(WETH).balanceOf(position));
        ...
        // Swap WETH to USDT with high slippage tolerance
        IPosition(position).swapTokenByPosition(WETH, USDT, 100 ether, 10000);
    ...
    function test_position_repay_with_collateral_swap() public {
        ...
        // Test repaying using WETH collateral through lending pool
        ...
        // Call repayWithSelectedToken through lending pool with WETH
        // This should trigger internal swap from WETH to USDT in the position
        ILendingPool(lendingPool).repayWithSelectedToken(10e6, WETH, false, alice, 10000);
    ...
    function test_position_swap_authorization_issue() public {
        ...
        console.log("WETH in position:", IERC20(WETH).balanceOf(position));
        ...
        try IPosition(position).swapTokenByPosition(WETH, USDT, 100 ether, 10000) {
    ...
    function test_position_repay_collateral_direct() public {
        ...
        // Test repaying using WETH collateral with high slippage tolerance
        vm.startPrank(alice);
        ...
        // Repay using WETH collateral through lending pool - this should swap internally
        // The position contract should handle the swap from WETH to USDT
        ILendingPool(lendingPool).repayWithSelectedToken(5e6, WETH, false, alice, 10000);
        ...
        // Try another repayment with WETH
        ILendingPool(lendingPool).repayWithSelectedToken(5e6, WETH, false, alice, 10000);
